...
[ -f "$SNAPSHOT_DIR"/nats.tar.bz2 ] || error "no nats.tar.bz2"
...
sudo docker run --rm -ti -v `readlink -f $SNAPSHOT_DIR`:/host:ro -v cyberway-nodeos-data:/data:rw cyberway/cyberway:$CYBERWAY_VERSION tar -xPvf /host/nodeos.tar.bz2
sudo docker run --rm -ti -v `readlink -f $SNAPSHOT_DIR`:/host:ro -v cyberway-mongodb-data:/data:rw cyberway/cyberway:$CYBERWAY_VERSION tar -xPvf /host/mongodb.tar.bz2
sudo docker run --rm -ti -v `readlink -f $SNAPSHOT_DIR`:/host:ro -v cyberway-nats-data:/data:rw cyberway/cyberway:$CYBERWAY_VERSION tar -xPvf /host/nats.tar.bz2
